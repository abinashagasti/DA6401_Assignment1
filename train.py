...
nn.weights = np.load("best_weights_config1_ce.npy", allow_pickle=True).item()
nn.biases = np.load("best_biases_config1_ce.npy", allow_pickle=True).item()

y_pred, accurate_preds, y_probs = return_predicted_output(nn, x_test, y_test)
print(f"Test accuracy: {(accurate_preds/y_test.shape[0])*100:.4f}")

create_confusion_matrix(y_test, y_pred, wandb)
